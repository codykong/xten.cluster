...
public class NetUtils {
    ...
    public static String getHostIPSafe(){
        try {
            return InetAddress.getLocalHost().getHostAddress();
    ...
    public static InetAddress getNetAddress() {
        ...
        try {
            ...
            if(inetAdds.length > 1 && inetAdds[1].isSiteLocalAddress() && inetAdds[1].getHostAddress().indexOf(":") == -1)
                return inetAdds[1];
            else if(inetAdds.length > 0 && inetAdds[0].isSiteLocalAddress() && inetAdds[0].getHostAddress().indexOf(":") == -1)
                return inetAdds[0];
            else return null;
        } catch (UnknownHostException e) {
            LOG.info("UnknownHostException:"+e.getMessage(),e);
